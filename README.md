...
## [2026-04-21] Текущий статус работ (детально)

Этот блок фиксирует текущее состояние разработки на ветке `feature/sport-split-uart`.

Что сделано:
1. Переведена связь с TX-модулем на split UART ESP32 (`GPIO32` TX, `GPIO33` RX, `400000` baud).
2. Добавлена расширенная диагностика CRSF:
   - `TXDIAG` (bytes/in/fwd/bad/addr/std),
   - `TXTYPES`,
   - RAW-дампы входящих/исходящих кадров,
   - расширенные сообщения CRC/формата.
3. Добавлена фильтрация эхо-кадров.
4. Добавлены доработки под ELRS v4+:
   - Device API запросы с origin `0xEA` и `0xEF`,
   - расширенный запрос параметров для диагностики.
5. Выполнены многократные циклы: правка -> сборка -> прошивка -> анализ serial-логов.

Какие шаги предприняты:
1. Отладка single-wire варианта S.Port.
2. Переход на схему с внешним инвертором и раздельными RX/TX.
3. Проверка по serial логам, dashboard и API.
4. Анализ CRC BAD, смешанных RAW кадров и эхо/петли.
5. Проверка влияния WiFi AP/STA при рабочем TX-модуле.

Текущий результат:
1. Поток данных идет (`in > 0`), пересылка идет (`fwd > 0`).
2. В канале остаются CRC/структурные ошибки и смешанные кадры.
3. Чтение/применение части параметров TX через dashboard пока нестабильно.
4. Текущий блокер: целостность физического тракта CRSF/S.Port (инверсия, наводки, эхо), а не только web-логика.

Следующий шаг:
1. Довести физический тракт до чистого двустороннего обмена.
2. После стабилизации канала завершить подстройку Device API парсинга/фильтрации.
